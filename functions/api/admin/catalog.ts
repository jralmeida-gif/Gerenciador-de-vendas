...
export const onRequestPost: PagesFunction<AuthEnv> = async ({ request, env }) => {
  ...
  if (action === 'delete') {
    ...
    await env.DB.batch([
      env.DB.prepare(`DELETE FROM ${table} WHERE name = ?`).bind(name),
  ...
  if (action === 'rename') {
    ...
    const statements: D1PreparedStatement[] = [
      env.DB.prepare(`UPDATE ${table} SET name = ?, ${column} = ?, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE name = ?`).bind(name, value, oldName),
      ...
      env.DB.prepare(`DELETE FROM ${mirror} WHERE name = ?`).bind(name),
      env.DB.prepare(`UPDATE ${mirror} SET name = ?, ${column} = ?, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE name = ?`).bind(name, value, oldName),
    ];
    ...
    if (entity === 'produto') {
      statements.push(
        ...
        env.DB.prepare('DELETE FROM user_metas WHERE product = ?').bind(name),
        env.DB.prepare('UPDATE user_metas SET product = ?, updated_at = datetime(\'now\') WHERE product = ?').bind(name, oldName),
        env.DB.prepare('DELETE FROM user_monthly_metas WHERE product = ?').bind(name),
  ...
  const existing = await env.DB.prepare(`SELECT name FROM ${table} WHERE name = ?`).bind(name).first<{ name: string }>();
  const statements: D1PreparedStatement[] = [
    env.DB.prepare(`INSERT INTO ${table} (name, ${column}, updated_at) VALUES (?, ?, strftime('%Y-%m-%dT%H:%M:%fZ','now'))
      ON CONFLICT(name) DO UPDATE SET ${column} = excluded.${column}, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now')`).bind(name, value),
    env.DB.prepare(`DELETE FROM ${mirror} WHERE name = ?`).bind(name),
    env.DB.prepare(`INSERT INTO ${mirror} (user_id, name, ${column}, updated_at)
      SELECT id, ?, ?, strftime('%Y-%m-%dT%H:%M:%fZ','now') FROM users`).bind(name, value),
  ];
  await env.DB.batch(statements);
